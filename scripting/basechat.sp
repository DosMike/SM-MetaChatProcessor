...
void SendChatToAll(int from, const char[] message)
{
	...
	mcpSenderFlag flags = mcpSenderNone;
	if (from) {
		if (GetClientTeam(from)<2) flags = mcpSenderSpectator;
		else if (!IsPlayerAlive(from)) flags = mcpSenderDead;
	}
	MCP_SendChat(from, recipients, message, flags, mcpTargetAll, mcpMsgGrouptagColor|mcpMsgIgnoreNameColor, "\x04");
	delete recipients;
}
...
void SendChatToAdmins(int from, const char[] message)
{
	...
	MCP_FindClientsByAccess("sm_chat", ADMFLAG_CHAT, recipients);
	mcpSenderFlag flags = mcpSenderNone;
	if (from) {
		if (GetClientTeam(from)<2) flags = mcpSenderSpectator;
		else if (!IsPlayerAlive(from)) flags = mcpSenderDead;
	}
	MCP_SendChat(from, recipients, message, flags, fromAdmin ? mcpTargetAdmin : mcpTargetToAdmin, mcpMsgGrouptagColor, "\x04");
	delete recipients;
}
...
void SendPrivateChat(int client, int target, const char[] message)
{
	...
	mcpSenderFlag flags = mcpSenderNone;
	if (client) {
		if (GetClientTeam(client)<2) flags = mcpSenderSpectator;
		else if (!IsPlayerAlive(client)) flags = mcpSenderDead;
	}
	MCP_SendChat(client, recipients, message, flags, mcpTargetDirect, mcpMsgGrouptagColor, "\x04", target);
	delete recipients;
